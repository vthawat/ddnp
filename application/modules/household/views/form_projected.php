<form method="post" class="form-horizontal" action="">
    ...
    <div class="col-sm-10"><br>
		...
		<?php if(!empty($potentiality)) foreach($potentiality as $item):?>
			...
			<?php if(empty($this->require_potential_list->get_one($id,$item->ID))):?>
  ...
  <div class="form-group bg-gray <?php if(form_error('PROBLEM')) print 'has-error'?>">
 ...
 <div class="form-group bg-gray <?php if(form_error('PROJECT_NAME')) print 'has-error'?>">
  ...
  <div class="form-group bg-gray <?php if(form_error('AREA_TARGET_VILLAGE')) print 'has-error'?>">
  	<label for="area_target" class="col-sm-2 control-label">พื้นที่ดำเนินการ/หมู่บ้าน*</label>
  	 <div class="col-sm-10">
  	 	<?php if(empty($Project)) $area_target=set_value('AREA_TARGET_VILLAGE');
				else $area_target=$Project->AREA_TARGET_VILLAGE;
    	?>
  	 	<textarea id="area_target" name="AREA_TARGET_VILLAGE" class="form-control"><?=$area_target?></textarea>
  	 </div>
  	  <span class="help-block">ตัวอย่าง หมู่ที่1,หมู่ที่2</span>
  </div>
 <div class="form-group bg-gray <?php if(form_error('BUDGET')) print 'has-error'?>">
   ...
   <div class="form-group bg-gray <?php if(form_error('RESPONSIBLE')) print 'has-error'?>">
