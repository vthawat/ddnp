            ...
            <li class="treeview">
              <a href="<?=base_url($this->router->fetch_class())?>"><i class='fa fa-book text-green fa-fw'></i><span>ข้อมูลความต้องการพัฒนา</span> <i class="fa fa-angle-left pull-right"></i></a>
              <ul class="treeview-menu">
              	<?php if(!empty($set_province)) foreach($set_province as $item):?>
                <li><a href="<?=base_url('requiredev/get/'.$item->ID)?>"><i class='fa fa-circle-o text-green fa-fw'></i><?=$this->province->desc.$item->PROVINCE_NAME?></a></li>
                <?php endforeach;?>
              </ul>
            </li>
	<!--
